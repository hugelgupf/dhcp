...
func TestWithOptionModifier(t *testing.T) {
	d, err := New()
	require.NoError(t, err)
	d = WithOption(OptDomainName("slackware.it"))(d)
...
func TestUserClassModifier(t *testing.T) {
	d, err := New()
	require.NoError(t, err)
	d = WithUserClass([]byte("linuxboot"), false)(d)

	expected := []byte{
		'l', 'i', 'n', 'u', 'x', 'b', 'o', 'o', 't',
	}
	require.Equal(t, expected, d.GetOption(OptionUserClassInformation))
}

func TestUserClassModifierRFC(t *testing.T) {
	d, err := New()
	require.NoError(t, err)
	d = WithUserClass([]byte("linuxboot"), true)(d)
	expected := []byte{
		9, 'l', 'i', 'n', 'u', 'x', 'b', 'o', 'o', 't',
	}
	require.Equal(t, expected, d.GetOption(OptionUserClassInformation))
...
func TestWithNetboot(t *testing.T) {
	d, _ := New()
	d = WithNetboot(d)
	require.Equal(t, "Parameter Request List: [TFTP Server Name, Bootfile Name]", GetParameterRequestList(d.Options).String())
}

func TestWithNetbootExistingTFTP(t *testing.T) {
	d, _ := New()
	d.AddOption(OptParameterRequestList(OptionTFTPServerName))
	d = WithNetboot(d)
	require.Equal(t, "Parameter Request List: [TFTP Server Name, Bootfile Name]", GetParameterRequestList(d.Options).String())
}
...
func TestWithNetbootExistingBootfileName(t *testing.T) {
	d, _ := New()
	d.AddOption(OptionBootfileName)
	d = WithNetboot(d)
	require.Equal(t, "Parameter Request List: [TFTP Server Name, Bootfile Name]", GetParameterRequestList(d.Options).String())
}
...
func TestWithNetbootExistingBoth(t *testing.T) {
	d, _ := New()
	d.AddOption(OptionBootfileName, OptionTFTPServerName)
	d = WithNetboot(d)
	require.Equal(t, "Parameter Request List: [TFTP Server Name, Bootfile Name]", GetParameterRequestList(d.Options).String())
}*/
...
func TestWithLeaseTime(t *testing.T) {
	d := &DHCPv4{}
	d = WithLeaseTime(3600 * time.Second)(d)
